...
public class MouseController : MonoBehaviour {
	...
	void FixedUpdate ()
    {
        ...
        {
            if(initialMousePosition.x > Input.mousePosition.x + cameraOffSet && transform.position.x < CameraMoveLimitTop.position.x)
            {
            ...
            if (initialMousePosition.x < Input.mousePosition.x - cameraOffSet && transform.position.x > CameraMoveLimitBottom.position.x)
            {
                transform.position = Vector3.Lerp(transform.position, transform.position - new Vector3(cameraOffSet, 0, 0), Time.deltaTime * smoothing);
            }
            if (initialMousePosition.y > Input.mousePosition.y + cameraOffSet && transform.position.z < CameraMoveLimitTop.position.z)
            {
            ...
            if (initialMousePosition.y < Input.mousePosition.y - cameraOffSet && transform.position.z > CameraMoveLimitBottom.position.z)
            {
                transform.position = Vector3.Lerp(transform.position, transform.position - new Vector3(0, 0, cameraOffSet), Time.deltaTime * smoothing);
            }
